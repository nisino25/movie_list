<template>
  <div>
    <span>Search movies</span><br><br>
  </div>
  <div>
    <form  @submit.prevent="searchMovie" >
      <input type="text" placeholder="Type to find a movie " v-model="MovieNum">&nbsp;
    ...
    <div v-if="!(results.poster_path === '')">
      <div v-if="condition !== 'error'">  
        <ul>
          <li>
            <div class="reusult">
              <strong>{{ results.original_title }}</strong><br><br>
              ...
              <div v-else>
                <a v-bind:href="tmdbLink" target="_blank">Check this movie at TMDB</a>&nbsp;&nbsp;
              </div>
      ...
      <div v-else>
        <img class="poster" :src="'https://image.tmdb.org/t/p/w400' + results.poster_path" alt="" v-if="results.poster_path !== null"  @load="isImageLoaded = true">
      </div>

      <div class="overview" v-if="results.overview !== null">
    ...
    <div v-if="condition === 'error'">
      <div v-if="condition !== 'searching'">
        <strong>Can't find the movie with the id</strong>
      </div>
    </div>
  </div>
</template>
...
// const API_KEY = `3019330967bc149f12628b6c43bd5a32`
// const SEARCH_PATH =`https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&query=`
// const NUM_FIND = `https://api.themoviedb.org/3/movie/${NUM}?api_key=3019330967bc149f12628b6c43bd5a32`
// const firstPart = `https://api.themoviedb.org/3/search/`
// const secondePart = `?api_key=3019330967bc149f12628b6c43bd5a32`
// let MovieNum = 500;
  ...
  data() {
   ...
   methods: {
     async searchMovie() {
      ...
      const json = await res.json(`${this.firstPart}${this.MovieNum}${this.secondePart}`)
      console.log(json)
    ...
    async GoodRating(){
      ...
      while(this.condition !== 'randomSuccess'){
        ...
        console.log(json)
        this.results = json
        this.tmdbLink = this.tmdbLink + this.results.original_title

        this.condition = 'searching'
        console.log(json)
        if(this.results.success !== false){
          if(this.results.vote_average >= 7 && this.results.vote_average !== 0 && this.results.vote_average !== 10){
            this.condition = 'randomSuccess'
            this.isFetchingMovie = false;
          }else{
            this.count++;
    ...
    async popMovie(){
      ...
      while(this.condition !== 'randomSuccess'){
        ...
        console.log(json)
        if(this.results.success !== false){
          if(this.results.popularity >= 10 && this.results.popularity!== 0){
            this.condition = 'randomSuccess'
            this.isFetchingMovie = false;
          }else{
            this.count++;
